...
node_t *make_node(node_t *node, elem_t Key, elem_t elem){
...
void tree_insert2(node_t *node, tree_key_t key, elem_t elem, element_comp_fun cmp_f, element_copy_fun cpy_f){
  node_t *p_new_node = NULL;
  int c = cmp_f(key, node->key);

  if(c < 0){
    if (node->left != NULL){
      tree_insert2(node->left, key, elem, cmp_f, cpy_f);
    }
    else{
      elem = (cpy_f) ? cpy_f(elem) : elem;
      node->left =  make_node(p_new_node, key, elem);
    }
  }
  else if(c > 0){
    if(node->right != NULL){
      tree_insert2(node->right, key, elem, cmp_f, cpy_f);
    }
    else{
      elem = (cpy_f) ? cpy_f(elem) : elem;
      node->right = make_node(p_new_node, key, elem);
    }
  }
}
...
bool tree_insert(tree_t *tree, tree_key_t key, elem_t elem){
  ...
  if(tree->rootnode == NULL){
    ...
    tree_insert2(tree->rootnode, key, elem, cmp_f, cpy_f);
    return true;
  } 
}
